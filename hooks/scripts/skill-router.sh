...
echo "$PROMPT" | grep -qiE "interface|criar tela|criar página|criar componente|design system|design do sistema|dashboard|landing page|visual|layout|mobile.first|ui|ux|frontend|shadcn|tailwind|estilo|cores|tipografia|tema" && suggest "frontend-design"
...
# Fase 09 — Handoff
echo "$PROMPT" | grep -qiE "documentação final|readme|entregar|handoff|cliente|finalizar projeto|concluir projeto|débito técnico documentar|projeto pronto|adr|decisão de arquitetura|entregar para o cliente" && suggest "project-handoff"
